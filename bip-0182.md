...
| ------------------ | ------------------------ | ----------------------------------------- |
| Utreexo_Tag_V1     | 64-byte array            | The first copy of the version tag to be prepended to the leafhash |
| Utreexo_Tag_V1     | 64-byte array            | The second copy of the version tag to be prepended to the leafhash |
| BlockHash          | 32-byte array            | The hash of the block in which this transaction was confirmed |
...
the check of BIP-0065 CLTV. In non-Utreexo nodes, the block height is stored locally
...
This data is also currently stored locally as a part of the UTXO set for non-Utreexo nodes.

##### Amount

This field is added to commit to the value of the UTXO. With non-Utreexo nodes, this
...
but since Utreexo nodes don't store this data, it must be provided.
...
transactions shared the same TXID, the transaction outputs of the succeeding
...
By utilizing the Utreexo accumulator, we're able to cap the storage required for a UTXO set commitment at $O(log(N))$.
...
Utreexo nodes are fully backwards compatible with non-Utreexo nodes as they will follow the same chain tip as the non-Utreexo nodes.
