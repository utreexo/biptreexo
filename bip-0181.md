...
The roots in this example are 06, 10, and 12.
...
**treerows(numleaves):** Returns the minimum number of bits required to represent `numleaves - 1`. This corresponds to the index of the largest tree in the forest. Returns `0` if `numleaves` is `0`.
...
For a fully filled tree with $2^n$ leaves, the labels for all leaves start with `0` in the first position and all inner nodes start with `1`. Shifting the child position right by one and setting the top bit therefore gives the parent position.
