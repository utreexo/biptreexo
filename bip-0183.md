...
Bridge nodes are nodes that can add inclusion proofs to mempool transactions and support the same set of messages as CSNs. They do not announce themselves as bridge nodes, although their network behavior may reveal that they are well-connected CSNs or bridge nodes.
...
Conventionally, IBD is done by a headers-first block download, in which the node downloads all the Bitcoin block headers, verifies that they connect, and follows up by downloading the block data for validation.

The image below illustrates how a non-Utreexo node would perform the IBD.
...
Hence, a Utreexo node will send a `getuproof` message along with the `getdata` message for a given block.
...
The image below illustrates how a Utreexo node would perform the IBD.
...
The image below illustrates how a Utreexo node would perform a bandwidth efficient IBD.
...
The image below illustrates how a non-Utreexo node would relay transactions.
...
When announcing a transaction to a Utreexo node with an `inv` message, we append `utreexoproofhash` inventory vectors to represent the Merkle tree positions for each of the UTXOs being referenced in the inputs of the transaction.
...
These positions are then sent over in the `getdata` message as another inventory vector.

The image below illustrates how a Utreexo node would relay transactions.
...
The image below illustrates how a Utreexo node would relay transactions with multiple inventory vectors of the type `utreexoproofhash`.
...
Note that an inventory vector of type `utreexoproofhash` MUST be appended to the transaction inventory vector.

The image below illustrates how a Utreexo node would relay multiple transactions.

![Utreexo TX relay with multiple transactions](bip-0183/utreexo-tx-relay-with-multiple-txs.png)
...
1. Node A sends an `inv` message or a block header to Node B.
2. Node B makes a `getdata` request for the block.
3. Node B makes a `getuproof` request for the block.
...
Since there's no commitment to anything in a `usummary` message, the information given in it can be false.
...
Several new data structures and messages are introduced to make the IBD and transaction relay possible.
...
| Field        | Type                | Description                                                                                                                                                                                                                                                    |
|--------------|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| block height | uint32              | The block height for this set of Utreexo TTL values                                                                                                                                                                          |
...
Each of the target locations represents the position of the leaf data at the same index.
While each leaf data represents a UTXO in a given block, not all are added as per [Utreexo - Validation Layer](bip-0182.md#excluded-utxos-from-the-accumulator).
...
| Field                      | Type                         | Description                                                                                                                                                                                                     |
|----------------------------|------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| transaction                | MSG_TX                       | The Bitcoin transaction. Unconfirmed inputs are marked by shifting the index left by 1 and setting the LSB                                                                                                      |
| length of the proof hashes | varint                       | The length of the proof hashes                                                                                                                                                                                  |
...
In order to start the 1601 through 2400 IBD task, however, the node should know what the state of the UTXO set is at block 1600, so that it can validate and modify the accumulator.
...
`NODE_UTREEXO` signals that the node understands Utreexo and will serve inclusion proofs for advertised transactions and blocks.
...
|------------------------------------------------- |--------------------------------------------------------------------------------------------------------------------------------------|
| NODE_NETWORK, NODE_UTREEXO_ARCHIVE, NODE_UTREEXO | Historical blocks + inclusion proofs for historical blocks + inclusion proofs for transactions and new blocks                        |
| NODE_NETWORK_LIMITED, NODE_UTREEXO               | Latest 288 blocks + inclusion proofs for latest 288 blocks + inclusion proofs for transactions and new blocks                        |
| NODE_UTREEXO                                     | Inclusion proofs for transactions and new blocks                                                                                     |
...
In order to differentiate nodes that serve just the historical Utreexo proofs and not the blocks, we needed to create a separate service bit.
